...
class CodeTimer:
    ...
    def __exit__(self, exc_type, exc_value, exc_tb):
        ...
        if not self._no_print:
            print(f"Code execution took {self.format_time(self.ns, self._min_digits)}.")
    
    @classmethod
    def format_time(cls, time_ns: int, min_digits: int) -> str:
        """Automatically selects the suitable time unit and returns a formatted string."""
        n_digits = len(str(time_ns))
        n_divisions = min((n_digits - 1) // 3, len(cls.UNITS) - 1)   # How many times to divide by 1000
        digits_after_division = n_digits - 3 * n_divisions
        decimal_places = max(0, min_digits - digits_after_division)
        fmt = ",." + str(decimal_places) + "f"    # How many digits spill over behind the decimal place
